 ...
 *
 *****************************************************************************/
#define _RTL8733BS_RECV_C_

#include <drv_types.h>		/* PADAPTER and etc. */
#include <hal_data.h>		/* HAL_DATA_TYPE */
#include "../../hal_halmac.h"	/* BIT_ACRC32_8733B and etc. */
#include "../rtl8733b.h"	/* rtl8733b_rxdesc2attribute(), rtl8733b_c2h_handler_no_io() */
 ...
 *	Pointer of _pkt, otherwise NULL.
 */
_pkt *rtl8733bs_alloc_recvbuf_skb(struct recv_buf *recvbuf, u32 size)
{
	_pkt *skb;
	...
	if (skb) {
		...
		rtl8733bs_free_recvbuf_skb(recvbuf);
	}
...
void rtl8733bs_free_recvbuf_skb(struct recv_buf *recvbuf)
{
...
static _pkt *prepare_recvframe_pkt(struct recv_buf *recvbuf, union recv_frame *recvframe)
{
	...
	desc_size = rtl8733b_get_rx_desc_size(recvbuf->adapter);

	attrib = &recvframe->u.hdr.attrib;
	skb_len = attrib->pkt_len;
	if (rtl8733b_rx_fcs_appended(recvbuf->adapter))
	 ...
	 * In 8733B, drvinfo_sz = 32, RXDESC_SIZE = 24, 1536 + 32 + 24 = 1592.
...
static u8 recvbuf_handler(struct recv_buf *recvbuf)
{
	...
	recvpriv = &p->recvpriv;
	ptr = recvbuf->pdata;
	desc_size = rtl8733b_get_rx_desc_size(p);
	...
	while (ptr < recvbuf->ptail) {
		...
		/* rx desc parsing */
		attrib = &recvframe->u.hdr.attrib;
		rtl8733b_rxdesc2attribute(attrib, ptr);

		/* drop recvbuf if pkt_len of rx desc is too small */
		pkt_len = attrib->pkt_len;
		if (pkt_len && rtl8733b_rx_fcs_appended(recvbuf->adapter)) {
		...
		if (!rtw_hal_rcr_check(p, BIT_ACRC32_8733B)
		    && attrib->crc_err) {
		...
		if ((attrib->crc_err) || (attrib->icv_err)) {
			...
			ptr += desc_size;
			if (rtl8733b_rx_ba_ssn_appended(p))
...
s32 rtl8733bs_recv_hdl(_adapter *adapter)
{
	...
	do {
		...
		c2h = GET_RX_DESC_C2H_8733B(recvbuf->pdata);
		if (c2h)
			rtl8733b_c2h_handler_no_io(adapter, recvbuf->pdata, recvbuf->len);
		else
...
static void recv_tasklet(unsigned long priv)
{
	...
	ret = rtl8733bs_recv_hdl(adapter);
 ...
 * 2. recv tasklet
 */
s32 rtl8733bs_init_recv_priv(PADAPTER adapter)
{
 ...
 * 2. recv tasklet
 */
void rtl8733bs_free_recv_priv(PADAPTER adapter)
{
	struct registry_priv *regsty = &adapter->registrypriv;
	u32 i, n;
	struct recv_priv *precvpriv;
...
void rtl8733bs_rxhandler(PADAPTER adapter, struct recv_buf *recvbuf)
{
	struct recv_priv *recvpriv;
