 ...
 *
 *****************************************************************************/
#define _RTL8733BS_HALINIT_C_

#include <drv_types.h>		/* PADAPTER, basic_types.h and etc. */
#include <hal_data.h>		/* HAL_DATA_TYPE */
#include "../../hal_halmac.h"	/* rtw_halmac_query_tx_page_num() */
#include "../rtl8733b.h"	/* rtl8733b_hal_init(), rtl8733b_phy_init_haldm() and etc. */
	...
	do {
		...
		if ((cpwm_orig ^ cpwm_now) & 0x80) {
			...
			RTW_INFO("%s: polling cpwm ok when leaving IPS in FWLPS state,"
				 ...
				 cpwm_orig, cpwm_now, rtw_read8(a, REG_CR_8733B));
...
static u8 fw_ips_enter(struct _ADAPTER *a)
{
	...
	do {
		rtw_mdelay_os(10);
		val8 = rtw_read8(a, REG_HMETFR_8733B);
		cnt++;
		if (!val8)
			break;
		...
		if (rtw_get_passing_time_ms(start_time) > 100) {
			...
			RTW_WARN("MAC_1C0=0x%08x, MAC_1C4=0x%08x, MAC_1C8=0x%08x, MAC_1CC=0x%08x\n",
				 ...
				 rtw_read32(a, 0x1c8), rtw_read32(a, REG_HMETFR_8733B));
#endif /* DBG_CHECK_FW_PS_STATE */
			goto exit;
		}
	} while (1);
...
u32 rtl8733bs_init(PADAPTER adapter)
{
	u8 ok = _TRUE;
	PHAL_DATA_TYPE hal;
...
void rtl8733bs_init_default_value(PADAPTER adapter)
{
	PHAL_DATA_TYPE hal;
	...
	rtl8733b_init_default_value(adapter);
