 ...
 *	For MI call.
 */
s32 rtl8733bs_dequeue_writeport(PADAPTER adapter)
{
 ...
 */
s32 rtl8733bs_xmit_buf_handler(PADAPTER adapter)
{
...
static s32 xmit_xmitframes(PADAPTER adapter, struct xmit_priv *pxmitpriv)
{
	...
	/*
	 * max_agg_num should never exceed OQT_ENTRY_AC_8733B(32 entry)
	 * For now, 8733bs can get best Tx TP with max_agg_num = 0x7
	 */
	max_agg_num = 0x7;
	/* max_agg_num = 0xFF;
		...
		while (rtw_end_of_queue_search(sta_phead, sta_plist) == _FALSE) {
			...
			while (rtw_is_list_empty(frame_phead) == _FALSE) {
				...
				if ((NULL == pxmitbuf)
					...
					if (pxmitbuf) {
						if (pxmitbuf->len > 0 && pxmitbuf->priv_data) {
							...
							pframe->agg_num = k;
							pxmitbuf->agg_num = k;
							rtl8733b_update_txdesc(pframe, pframe->buf_addr);
				...
				if (ret == _FAIL) {
					...
					k++;
					if (k != 1)
						rtl8733b_update_txdesc(pxmitframe, pxmitframe->buf_addr);
...
thread_return rtl8733bs_xmit_thread(thread_context context)
{
	s32 ret;
	PADAPTER adapter;
	struct xmit_priv *pxmitpriv;
 ...
 *	_FAIL		fail
 */
s32 rtl8733bs_mgnt_xmit(PADAPTER adapter, struct xmit_frame *pmgntframe)
{
	s32 ret = _SUCCESS;
	...
	rtl8733b_update_txdesc(pmgntframe, pmgntframe->buf_addr);
 ...
 */
#ifdef CONFIG_RTW_MGMT_QUEUE
s32 rtl8733bs_hal_mgmt_xmit_enqueue(PADAPTER adapter, struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv;
	s32 ret;
 ...
 *	_FALSE	fail
 */
s32 rtl8733bs_hal_xmit_enqueue(PADAPTER adapter, struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv;
	s32 ret;
 ...
 */
s32 rtl8733bs_hal_xmit(PADAPTER adapter, struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv;
	_irqL irql;
	s32 ret;
	...
	_enter_critical_bh(&pxmitpriv->lock, &irql);
	ret = rtl8733bs_hal_xmit_enqueue(adapter, pxmitframe);
	_exit_critical_bh(&pxmitpriv->lock, &irql);
	if (ret != _TRUE) {
 ...
 *
 */
s32 rtl8733bs_init_xmit_priv(PADAPTER adapter)
{
...
#endif
	rtl8733b_init_xmit_priv(adapter);
	return _SUCCESS;
}

void rtl8733bs_free_xmit_priv(PADAPTER adapter)
{
	struct xmit_priv *pxmitpriv;
	struct xmit_buf *pxmitbuf;
	_queue *pqueue;
